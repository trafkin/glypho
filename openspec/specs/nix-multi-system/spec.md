# nix-multi-system Specification

## Purpose

The Nix flake exposes working `glypho` packages and checks on `x86_64-linux`, `aarch64-linux`, `x86_64-darwin`, and `aarch64-darwin`, with per-system target/linker selection instead of Linux-only hardcoding.
